...
<div class="hero-dark" style="padding: 40px 50px;">
    <div class="hero-dark-content">
        <div class="hero-dark-title" style="font-size: 1.7rem;">
            🛡️ Actuarial Life Insurance Pricing Calculator
        </div>
    </div>
</div>
...
st.markdown(
    ...
    Simulating net premiums for a 
    <strong>{inputs['age']}-year-old</strong>
    with 
    <strong>{format_currency(inputs['sum_assured'])}</strong>
    coverage under the 
    <strong>{inputs['scenario_name']}</strong>
    mortality scenario.
    </p>
    """,
    unsafe_allow_html=True
...
with chart_col2:
    ...
    # Gender removed. Pass None instead of "All"
    render_mortality_rate_comparison(
        res['ages_axis'][:-1],
        res['base_rates'],
        res['improved_rates'],
        None
...
st.markdown(
    '<div class="glass-label">Expected Claims Projections (First 10 Years)</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    ...
    This table projects the expected annual claim payment amounts
    based on the survival and mortality probabilities.
    </p>
    """,
    unsafe_allow_html=True
)
...
render_claims_projection_table(
    res['claims_projection'][:10]
)