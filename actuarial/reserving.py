...
Gross_Premium_Reserve_Age40_Term30.xlsx illustration:
    ...
    tV = SA x A^1(x+t : n-t)  +  PV(Future Expenses)_t  -  P x a(x+t : n-t)

Where:
    A^1(x+t : n-t)   = term assurance EPV factor at duration t (remaining term)
    a(x+t : n-t)     = annuity-due EPV factor at duration t (remaining term)
...
def calculate_reserve_schedule(
    ...
    gross_premium: if provided, use this premium instead of solving it via
        the equivalence principle. If None, P is solved using this module's
        own equivalence-principle formula matching the Excel exactly.

    Returns a dict with:
        gross_premium: the solved (or provided) gross annual premium
        schedule: list of dicts, one per duration t = 0..term
    """
    ...
    # --- Step 2: For each duration t, compute reserve components ---
    schedule = []
